...
class MultiDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val colorsBoolean: BooleanArray =
            colorComponents.map { it > 0 && savedInstanceState == null }.toBooleanArray()
        return AlertDialog.Builder(requireContext())
            .setTitle("ffff")
            .setMultiChoiceItems(colorItems, colorsBoolean) { dialog, which, isCheced ->
                val chekedPositions =
                    (dialog as AlertDialog).listView.checkedItemPositions // список выбраноых чекбоксов из встроеного листвью
                val color = Color.rgb(
                    booleanToColorComponent(chekedPositions[0]),
                    booleanToColorComponent(chekedPositions[1]),
                    booleanToColorComponent(chekedPositions[2])
                )
    ...
    companion object {
        @JvmStatic
        private val TAG = MultiDialogFragment ::class.java.simpleName

        @JvmStatic
        private val KEY_COLOR_RESPONSE = "KEY_COLOR_RESPONSE"
        ...
        fun show(manager: FragmentManager, color: Int) {
            val dialogFragment = MultiDialogFragment()
